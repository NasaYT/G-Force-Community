...
exports.run = async (client, message, args) => {
  ...
  .addField(`:book: **Exemplo**`, `\`${c.prefix}warnstaff @uGabriel#2193 lindao\``)
  ...
  // Requisitando a permissão de Administrador
    ...
    .setFooter(`Avisado por ${message.author.username}`)

    //
    membro.send(embed); // Enviando pro usuário
    canal.send(embed); // Enviando no canal
    message.delete();
    ...
    if(warnstaff[membro.id].warnstaff == 999) {
      let embedi = new Discord.MessageEmbed()

        .setTitle(`:warning: RD - BAN :warning:`)
        .setFooter(`Você foi banido da RedeDamon pelo Staff: ${message.author.username}`)
      
      await membro.send(embedi)
      message.guild.member(membro).ban(motivo);
      membro.send('Você foi banido da RedeDamon.')
      message.channel.send(`<@${membro.id}> foi banido.`)
    }
}
